<div align="center">

  [![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white)](https://www.oracle.com/java/)
  [![JavaFX](https://img.shields.io/badge/JavaFX-007396?style=for-the-badge&logo=java&logoColor=white)](https://openjfx.io/)

</div>


All in One can convert JSON, XML, CSV to XML, CSV, JSON.
This is a challenge from me to develop algorithms that translate data format from X to Y,
without using any API, and with a project structure that allows scaling and adding more features.

## THE UI
choice your data format and paste and click t convert and you will see the result in as ouput

<img src="/images/ui.png">

## THE VIDEO


## WHAT YOU NEED
1. Download Javafx FROM: **https://gluonhq.com/products/javafx/**
2. papier and pen to understand well the algorithms
3. In this project I also do translations using APIs **LIKE**: <br/>
    **org.json library and rapidapi API**

## HOW TO START
1. fork the project
2. download javaFx and org.json and add them to your lib folder
3. execute the file make.sh [contains commads used to compile and execute the project]


if you like project why not to give me star or if you have any feedbacks please share with me <br/> 
**made with <a href="https://www.linkedin.com/in/abdellah-karani-965928294/">me</a> With HUGE :|**

